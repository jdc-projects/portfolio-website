export const meta = {
  title: 'Homelab',
  description: 'A top-level overview of my homelab setup',
  thumbnail: 'https://images.unsplash.com/photo-1597138768744-9f97be8cdd64?&ixlib=rb-4.0.3',
  thumbnailAlt: 'A server rack',
  githubLink: 'https://github.com/jdc-projects/homelab',
}

import ArchitectureDiagram from './architecture.drawio.svg'
import CaptionedImage from 'components/CaptionedImage'
import ImageCollage from 'components/ImageCollage'

# Homelab

## Goal

To set up a software / infrastructure lab primarily for testing, experimentation, and learning.
It also needs to be able to host some useful services, such as file hosting and synchronisation.

I already have a server to use, so the solution needs to work on that.
This also means I can turn it off when not needed, and save compared to cloud hosting.

The system should also be maleable and adaptable to support changes, experimentation, and evolution.

## Technology

- [Terraform](https://www.terraform.io/), an infrastructure-as-code (IaC) system
- [Kubernetes](https://kubernetes.io/), a container orchestration / cluster platform
- [GitHub Actions](https://github.com/features/actions), a CI/CD platform
- [CloudNative-PG operator](https://cloudnative-pg.io/), which provides Postgres databases where needed
- [Keycloak](https://www.keycloak.org/), an identity / single-signon provider
- [Loki](https://grafana.com/oss/loki/), a log aggregator which feeds into Grafana
- [OwnCloud Infinite Scale](https://owncloud.com/infinite-scale), 'cloud storage' software
- [OpenEBS](https://openebs.io/), provides the Kubernetes storage backend
- [OpenLDAP](https://www.openldap.org/), an identity manager
- [Grafana operator](https://github.com/grafana/grafana-operator), providing a Grafana instance and configuration
- [Traefik](https://traefik.io/traefik/), an ingress controller and reverse proxy
- [Crowdsec](https://www.crowdsec.net/), a threat intelligence platform and WAF, integrated with Traefik
- [Vaultwarden](https://www.vaultwarden.net/), a password manager
- [Outline](https://www.getoutline.com/), a note taking / knowledge base application
- [KubeVirt](https://kubevirt.io/), a virtualisation platform
- [Velero](https://velero.io/), the cluster's backup solution

## Architecture

Each service / project will have its own namespace, keeping them reasonably separate.
This diagram is a highly simplified view of that architecture.

The only service that is explicitly included in the diagram is Traefik - as it is the ingress controller, it provides access to the other services and projects.
Additionally, the provisioning and management of the cluster through GitHub Actions and Terraform is included.

<CaptionedImage src={ArchitectureDiagram} caption='Architecture for my homelab' maw={800} />

## Skills

<ImageCollage imagesProps={[
  {src: 'https://raw.githubusercontent.com/kubernetes/kubernetes/master/logo/logo.svg', alt: "Kubernetes logo" },
  {src: 'https://www.svgrepo.com/show/374122/terraform.svg', alt: "Terraform logo" },
  {src: 'https://cdn.worldvectorlogo.com/logos/github-icon-2.svg', alt: "Github logo" },
]} />
